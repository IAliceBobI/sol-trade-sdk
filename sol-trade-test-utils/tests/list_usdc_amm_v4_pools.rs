//! 列出所有 USDC 相关的 Raydium AMM V4 Pool，区分 Token 和 Token2022
//!
//! 这是一个探索性测试，用于查询和展示 USDC 相关的 AMM V4 Pool 信息
//!
...
//!
//! 运行测试:
//!     cargo test --package sol-trade-test-utils list_usdc_amm_v4_pools_test -- --ignored

use sol_trade_test_utils::list_usdc_amm_v4_pools;

#[tokio::test]
#[ignore = "需要支持 getProgramAccounts 的 RPC 节点"]
async fn list_usdc_amm_v4_pools_test() {
    // 注意：本地测试节点可能没有 AMM V4 Pool 数据
    // 如需测试，请使用支持完整 RPC 功能的节点
    let rpc_url = "http://127.0.0.1:8899";

    // 使用便捷函数列出 USDC AMM V4 Pool
    let result = list_usdc_amm_v4_pools(rpc_url, Some(10)).await;

    // 如果本地节点不支持，测试会失败（这是预期的）
    match result {
        Ok(classification) => {
            println!("\n✅ 查询成功");
