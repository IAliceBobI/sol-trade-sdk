...
pub(crate) static MINT_TO_POOL_CACHE: Lazy<
    DashMap<solana_sdk::pubkey::Pubkey, solana_sdk::pubkey::Pubkey>,
...
pub(crate) static MINT_TO_POOLS_LIST_CACHE: Lazy<
    DashMap<solana_sdk::pubkey::Pubkey, Vec<(solana_sdk::pubkey::Pubkey, PoolState)>>,
> = Lazy::new(|| DashMap::with_capacity(MAX_CACHE_SIZE));

pub(crate) fn get_cached_pool_by_address(
...
pub(crate) fn get_cached_pool_address_by_mint(
    mint: &solana_sdk::pubkey::Pubkey,
) -> Option<solana_sdk::pubkey::Pubkey> {
    MINT_TO_POOL_CACHE.get(mint).map(|p| *p)
}

pub(crate) fn cache_pool_address_by_mint(
    mint: &solana_sdk::pubkey::Pubkey,
    pool_address: &solana_sdk::pubkey::Pubkey,
) {
...
pub(crate) fn get_cached_pools_list_by_mint(
    mint: &solana_sdk::pubkey::Pubkey,
) -> Option<Vec<(solana_sdk::pubkey::Pubkey, PoolState)>> {
    MINT_TO_POOLS_LIST_CACHE.get(mint).map(|p| p.clone())
}

#[expect(dead_code, reason = "预留用于未来缓存策略优化")]
pub(crate) fn cache_pools_list_by_mint(
    mint: &solana_sdk::pubkey::Pubkey,
    pools: &[(solana_sdk::pubkey::Pubkey, PoolState)],
) {
