...
impl RaydiumV4Parser {
    ...
    /// 获取指令相关的 Transfer 记录
    /// TODO: 需要从 inner instructions 中提取 Token Program 的 transfer 指令
    fn get_transfers_for_instruction(
        &self,
        _adapter: &TransactionAdapter,
        _outer_index: usize,
    ) -> Result<Vec<TransferRecord>, ParseError> {
        // TODO: 完整实现需要解析 inner instructions
        // 查找 Token Program 的 transfer/transferChecked 指令
        Ok(vec![])
    }
    ...
    fn determine_trade_type(
        ...
        let (input_transfer, output_transfer, trade_type) = if transfers.len() >= 2 {
            ...
            // 判断哪个是输入，哪个是输出
            if t0.source == user && t1.destination == user {
                if t0.mint == sol_mint {
                    (t0, t1, TradeType::Buy)
                } else {
                    (t0, t1, TradeType::Sell)
                }
            } else if t1.source == user && t0.destination == user {
                if t1.mint == sol_mint {
                    (t1, t0, TradeType::Buy)
                } else {
                ...
                return Err(ParseError::ParseFailed("无法确定交易方向".into()));
            }
        } else {
