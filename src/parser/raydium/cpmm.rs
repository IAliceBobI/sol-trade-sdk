...
impl RaydiumCpmmParser {
    ...
    /// 从账户列表中提取用户地址
    /// Raydium CPMM: 用户通常是 payer (accounts[0])
...
#[async_trait]
impl DexParserTrait for RaydiumCpmmParser {
    async fn parse(&self, adapter: &TransactionAdapter) -> Result<Vec<ParsedTradeInfo>, ParseError> {
        let mut trades = Vec::new();

        let program_id_str = DexProtocol::RaydiumCpmm.program_id();
        let program_pubkey = program_id_str.parse()
            .map_err(|_| ParseError::UnsupportedProtocol("Invalid Raydium CPMM program ID".to_string()))?;
        ...
        for instr in instructions {
            ...
            trades.push(ParsedTradeInfo {
                ...
                fee,
                fees: vec![],
                dex: DexProtocol::RaydiumCpmm.name().to_string(),
        ...
        if trades.is_empty() {
            return Err(ParseError::ParseFailed(
                "未找到有效的 Raydium CPMM 交易".to_string(),
            ));
        }
...
mod tests {
    ...
    #[test]
    fn test_raydium_cpmm_parser_creation() {
        let parser = RaydiumCpmmParser::new();
        assert_eq!(parser.protocol(), DexProtocol::RaydiumCpmm);
    }
