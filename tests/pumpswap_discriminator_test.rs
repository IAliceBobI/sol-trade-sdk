...
fn test_pumpswap_discriminators() {
    ...
    let buy_type = registry.identify(ParserDexProtocol::PumpSwap, &buy);
    assert!(
    ...
    // 验证 BUY 不是流动性操作
    assert!(
        !registry.is_liquidity_discriminator(ParserDexProtocol::PumpSwap, &buy),
        "BUY 不应该被识别为流动性操作"
    );

    // 验证 BUY 是 Swap 操作
    assert!(
        registry.is_swap_discriminator(ParserDexProtocol::PumpSwap, &buy),
        "BUY 应该被识别为 Swap 操作"
    );

    // SELL 操作的 discriminator (8 字节)
    let sell = [51, 230, 133, 164, 1, 127, 131, 173];
    let sell_type = registry.identify(ParserDexProtocol::PumpSwap, &sell);
    assert!(
    ...
    // 验证 SELL 不是流动性操作
    assert!(
        !registry.is_liquidity_discriminator(ParserDexProtocol::PumpSwap, &sell),
        "SELL 不应该被识别为流动性操作"
    );

    // 验证 SELL 是 Swap 操作
    assert!(
        registry.is_swap_discriminator(ParserDexProtocol::PumpSwap, &sell),
        "SELL 应该被识别为 Swap 操作"
    );

    // ADD_LIQUIDITY 操作的 discriminator (8 字节)
    let add_liq = [242, 35, 198, 137, 82, 225, 242, 182];
    let add_liq_type = registry.identify(ParserDexProtocol::PumpSwap, &add_liq);
    assert!(
        matches!(
            add_liq_type,
            sol_trade_sdk::parser::discriminators::InstructionType::AddLiquidity
        ),
    ...
    assert!(
        registry.is_liquidity_discriminator(ParserDexProtocol::PumpSwap, &add_liq),
        "ADD_LIQUIDITY 应该被识别为流动性操作"
    );

    // 验证 ADD_LIQUIDITY 不是 Swap 操作
    assert!(
        !registry.is_swap_discriminator(ParserDexProtocol::PumpSwap, &add_liq),
        "ADD_LIQUIDITY 不应该被识别为 Swap 操作"
    );

    // REMOVE_LIQUIDITY 操作的 discriminator (8 字节)
    let remove_liq = [183, 18, 70, 156, 148, 109, 161, 34];
    let remove_liq_type = registry.identify(ParserDexProtocol::PumpSwap, &remove_liq);
    assert!(
        matches!(
            remove_liq_type,
            sol_trade_sdk::parser::discriminators::InstructionType::RemoveLiquidity
        ),
    ...
    assert!(
        registry.is_liquidity_discriminator(ParserDexProtocol::PumpSwap, &remove_liq),
        "REMOVE_LIQUIDITY 应该被识别为流动性操作"
    );

    // 验证 REMOVE_LIQUIDITY 不是 Swap 操作
    assert!(
        !registry.is_swap_discriminator(ParserDexProtocol::PumpSwap, &remove_liq),
        "REMOVE_LIQUIDITY 不应该被识别为 Swap 操作"
    );

    // CREATE_POOL 操作的 discriminator (8 字节)
    let create_pool = [233, 146, 209, 142, 207, 104, 64, 188];
    let create_pool_type = registry.identify(ParserDexProtocol::PumpSwap, &create_pool);
    assert!(
        matches!(
            create_pool_type,
            sol_trade_sdk::parser::discriminators::InstructionType::CreatePool
        ),
    ...
    assert!(
        registry.is_liquidity_discriminator(ParserDexProtocol::PumpSwap, &create_pool),
        "CREATE_POOL 应该被识别为流动性操作"
    );

    // 验证 CREATE_POOL 不是 Swap 操作
    assert!(
        !registry.is_swap_discriminator(ParserDexProtocol::PumpSwap, &create_pool),
