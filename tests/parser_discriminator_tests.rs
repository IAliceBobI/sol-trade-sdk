use sol_trade_sdk::parser::discriminators::{DexProtocol, DiscriminatorRegistry, InstructionType};
...
fn test_raydium_clmm_unknown_discriminator() {
    ...
    let instr_type = registry.identify(DexProtocol::RaydiumClmm, &unknown_instruction);
...
fn test_raydium_clmm_liquidity_exclusion() {
    ...
    let instr_type = registry.identify(DexProtocol::RaydiumClmm, &open_position);
    ...
    // 验证未注册的指令不是流动性操作
    let unknown_instruction = [0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00, 0x11];
    assert!(!registry.is_liquidity_discriminator(DexProtocol::RaydiumClmm, &unknown_instruction));
}
...
fn test_pumpswap_buy_discriminator() {
    ...
    let instr_type = registry.identify(DexProtocol::PumpSwap, &buy_instruction);
...
fn test_pumpswap_sell_discriminator() {
    ...
    let instr_type = registry.identify(DexProtocol::PumpSwap, &sell_instruction);
...
fn test_pumpswap_liquidity_operations() {
    ...
    // CreatePool
    let create_pool = [233, 146, 209, 142, 207, 104, 64, 188];
    assert_eq!(registry.identify(DexProtocol::PumpSwap, &create_pool), InstructionType::CreatePool);
...
fn test_is_swap_discriminator() {
    ...
    let buy_instruction = [102, 6, 61, 18, 1, 218, 235, 234];
    assert!(registry.is_swap_discriminator(DexProtocol::PumpSwap, &buy_instruction));

    let sell_instruction = [51, 230, 133, 164, 1, 127, 131, 173];
    assert!(registry.is_swap_discriminator(DexProtocol::PumpSwap, &sell_instruction));

    // 流动性操作不应该被识别为 Swap
    let add_liquidity = [242, 35, 198, 137, 82, 225, 242, 182];
    assert!(!registry.is_swap_discriminator(DexProtocol::PumpSwap, &add_liquidity));
}
...
fn test_short_instruction_data() {
    ...
    let instr_type = registry.identify(DexProtocol::PumpSwap, &short_data);
