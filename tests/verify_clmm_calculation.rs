...
use sol_trade_sdk::{
    ...
    instruction::utils::raydium_clmm::{
        get_pool_by_address, get_tick_array_start_index, get_tick_arrays, quote_exact_in,
    },
...
async fn test_verify_clmm_full_calculation_with_rpc_quote() {
    ...
    for (pool_name, pool_address_str, amount_in) in pools_to_test {
        ...
        let fee_rate =
            match sol_trade_sdk::instruction::utils::raydium_clmm_types::amm_config_decode(
                &amm_config.data,
            ) {
        ...
        let zero_for_one =
            pool_state.token_mint1.to_string() == "So11111111111111111111111111111111111111112";
        ...
        let tick_arrays_formatted: Vec<(i32, Vec<(i32, i128, u128)>)> = match tick_arrays_result {
            Ok(tick_array_states) => {
                ...
                tick_array_states
                    .into_iter()
                    .map(|(start_index, tick_array_state)| {
        ...
        let rpc_quote = match quote_exact_in(&rpc, &pool_address, amount_in, zero_for_one).await {
            Ok(quote) => quote,
            Err(e) => {
        ...
        let error_rate =
            if rpc_output > 0 { (diff as f64 / rpc_output as f64) * 100.0 } else { 0.0 };
