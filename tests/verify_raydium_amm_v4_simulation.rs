...
//!
//! 运行测试:
//!     cargo nextest run verify_raydium_amm_v4_simulation -- --nocapture
...
async fn test_raydium_amm_v4_local_calc_vs_onchain_simulation() {
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("🔬 Raydium AMM V4 本地计算 vs 链上模拟对比测试");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");

    println!("📋 测试目标:");
    println!("   1. ✅ 验证本地计算的准确性（使用 AMM 恒定乘积公式）");
    ...
    println!("✅ Pool 状态获取成功");
    println!("   Coin Mint: {}", pool_state.coin_mint);
    println!("   PC Mint: {}", pool_state.pc_mint);
    println!("   Token Coin: {}", pool_state.token_coin);
    println!("   Token PC: {}\n", pool_state.token_pc);
    ...
    // 步骤 3: 本地计算（使用 AMM 公式）
    // ========================================
    println!("🧮 步骤 3: 本地计算（AMM 恒定乘积公式）");

    // AMM 公式: (input_amount * output_reserve) / (input_reserve + input_amount)
    // 这里简化计算，实际需要考虑手续费
    let local_output = if coin_reserve > 0 && pc_reserve > 0 {
        let input_reserve = coin_reserve;
        let output_reserve = pc_reserve;
    ...
    println!("✅ 本地计算结果: {} USDC (最小单位)\n", local_output);

    // ========================================
    // 步骤 3: 构造真实的 AMM V4 Swap 指令
    // ========================================
    println!("📡 步骤 3: 构造 AMM V4 Swap 指令");
...
#[tokio::test]
#[serial_test::serial]
async fn test_raydium_amm_v4_quote_exact_in_accuracy() {
    println!("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("🔬 AMM V4 quote_exact_in 准确性测试");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
    ...
    let local_quote = match quote_exact_in(&rpc, &pool_address, amount_in, true).await {
        Ok(quote) => {
    ...
    // 3. 构造交易指令
    println!("📡 步骤 2: 构造 AMM V4 Swap 指令");
    ...
    // 获取储备余额
    let coin_balance = match rpc.get_token_account_balance(&pool_state.token_coin).await {
        Ok(balance) => balance.amount.parse::<u64>().unwrap_or(0),
        Err(_) => {
            println!("❌ 无法获取 coin reserve\n");
            return;
        },
    };

    let pc_balance = match rpc.get_token_account_balance(&pool_state.token_pc).await {
        Ok(balance) => balance.amount.parse::<u64>().unwrap_or(0),
        Err(_) => {
            println!("❌ 无法获取 pc reserve\n");
            return;
        },
    };
    ...
    let amm_params = RaydiumAmmV4Params {
        ...
        token_coin: pool_state.token_coin,
        token_pc: pool_state.token_pc,
        coin_reserve: coin_balance,
        pc_reserve: pc_balance,
    };
